...
| --------------------------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ------- |
| `.github/workflows/reusable-node.yml`         | Node/TS CI: install → format → lint → typecheck → test → build                  | `working_directory`, `node_version`, `run_*` toggles, optional `test_cmd`                                                         | `contents: read`                                            | none    |
...
jobs:
  ci:
    ...
    with:
      working_directory: '.'
      test_cmd: npm run -s test:coverage
    secrets: inherit
```
