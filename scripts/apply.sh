...
# 1) CI caller + stack-specific baseline
if [[ "$TYPE" == "node" ]]; then
  cp "${ROOT_DIR}/scaffolds/node/ci.yml" "$TARGET/.github/workflows/ci.yml"
  cp "${ROOT_DIR}/scaffolds/node/dependabot.yml" "$TARGET/.github/dependabot.yml"
  ...
  [[ -f "$TARGET/.npmrc" ]] || cp "${ROOT_DIR}/configs/node/.npmrc" "$TARGET/.npmrc"
  [[ -f "$TARGET/.nvmrc" ]] || cp "${ROOT_DIR}/configs/node/.nvmrc" "$TARGET/.nvmrc"

elif [[ "$TYPE" == "python" ]]; then
  cp "${ROOT_DIR}/scaffolds/python/ci.yml" "$TARGET/.github/workflows/ci.yml"
  cp "${ROOT_DIR}/scaffolds/python/dependabot.yml" "$TARGET/.github/dependabot.yml"
...
if [[ "$REPO_BASELINE" == "true" ]]; then
  ...
  if [[ ! -f "$TARGET/.gitignore" ]]; then
    if [[ "$TYPE" == "node" ]]; then
      cp "${ROOT_DIR}/scaffolds/node/.gitignore" "$TARGET/.gitignore"
    else
      cp "${ROOT_DIR}/scaffolds/python/.gitignore" "$TARGET/.gitignore"
    fi
  fi
  [[ -f "$TARGET/LICENSE" ]] || cp "${ROOT_DIR}/LICENSE" "$TARGET/LICENSE"
